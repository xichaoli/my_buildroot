...
SKELETON_TARGET_GENERIC_COMPANY= $(call qstrip,$(BR2_TARGET_GENERIC_COMPANY))
SKELETON_TARGET_GENERIC_PRODUCT = $(call qstrip,$(BR2_TARGET_GENERIC_PRODUCT))
SKELETON_TARGET_GENERIC_RELEASE = $(call qstrip,$(BR2_TARGET_GENERIC_RELEASE))
SKELETON_TARGET_GENERIC_MAIN_VERSION = $(call qstrip,$(BR2_TARGET_GENERIC_MAIN_VERSION))
...
ifneq ($(SKELETON_TARGET_GENERIC_COMPANY),)
define SKELETON_SET_COMPANY
	mkdir -p $(TARGET_DIR)/etc
	echo "COMPANY:${SKELETON_TARGET_GENERIC_COMPANY}" > ${TARGET_DIR}/etc/os-release
endef
TARGET_FINALIZE_HOOKS += SKELETON_SET_COMPANY
endif

ifneq ($(SKELETON_TARGET_GENERIC_PRODUCT),)
define SKELETON_SET_PRODUCT
	mkdir -p $(TARGET_DIR)/etc
	echo "PRODUCT:${SKELETON_TARGET_GENERIC_PRODUCT}" >> ${TARGET_DIR}/etc/os-release
endef
TARGET_FINALIZE_HOOKS += SKELETON_SET_PRODUCT
endif

ifneq ($(SKELETON_TARGET_GENERIC_RELEASE),)
define SKELETON_SET_RELEASE
	mkdir -p $(TARGET_DIR)/etc
	echo "RELEASE:${SKELETON_TARGET_GENERIC_RELEASE}" >> ${TARGET_DIR}/etc/os-release
endef
TARGET_FINALIZE_HOOKS += SKELETON_SET_RELEASE
endif

ifneq ($(SKELETON_TARGET_GENERIC_MAIN_VERSION),)
define SKELETON_SET_VERSION
	mkdir -p $(TARGET_DIR)/etc
	echo "VERSION:${SKELETON_TARGET_GENERIC_MAIN_VERSION}.$(shell date +%y%m%d%H%M).$(ARCH)" >> ${TARGET_DIR}/etc/os-release
endef
TARGET_FINALIZE_HOOKS += SKELETON_SET_VERSION
endif
...
define SKELETON_SET_NETWORK_LOCALHOST
	( \
		...
		echo ;                                               \
		echo "# The common network interface";               \
		echo "source /conf/network/ifcfg-*";                 \
